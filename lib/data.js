...
function analyzeSegment(meta, allWeekly, weeks) {
  ...
  // Avoidable cost only counts segments whose cost has genuinely blown out past
  // the tolerance band — otherwise tiny per-tonne noise on big tonnages inflates it.
  const costBlownOut = latestAvgCPT > baselineCPT * THRESHOLDS.costBlowoutThresholdMultiplier;
  const avoidableCost = costBlownOut ? Math.max(0, latestTonnes * (latestAvgCPT - baselineCPT)) : 0;
...
// Avoidable cost accrues per blown-out WEEK (cost above the tolerance band), so a
// late blowout isn't averaged away over a long window and small per-tonne noise
// on large tonnages never inflates the headline.
function networkForWindow(routeAnalyses, weeks) {
  ...
  for (const id of Object.keys(routeAnalyses)) {
    ...
    tonnes += agg.tonnes;
    const ceiling = ra.baselineCPT * THRESHOLDS.costBlowoutThresholdMultiplier;
    for (const w of weeks) {
      const wd = ra.weeklyData.find(x => x.week === w);
      if (wd && wd.avgCPT != null && wd.totalTonnes != null && wd.avgCPT > ceiling) {
        avoidable += wd.totalTonnes * (wd.avgCPT - ra.baselineCPT);
      }
    }
    const isHot = (agg.avgDelay != null && agg.avgDelay > 4) || (agg.avgCPT != null && agg.avgCPT > ceiling);
    if (isHot) hot++;
  }
...
// ── KPI snapshot (shared by the History tab and the server-side cron) ────────
...
export const snapSig = s => `${s.rawRows}:${s.cleanTrips}:${Math.round(s.avoidableCost)}:${Math.round(s.networkTonnes)}:${s.flaggedSegments}`;
